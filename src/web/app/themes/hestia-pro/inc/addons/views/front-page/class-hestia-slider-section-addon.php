...
class Hestia_Slider_Section_Addon extends Hestia_First_Front_Page_Section {
	 ...
	 *
	 * @param string $link     Link for first button.
	 * @param string $button   Label for first button.
	 * @param string $link2    Link for second button.
	 * @param string $button2  Label for second button.
	 * @param int    $slide_nb Current slide number
	 */
	private function render_slide_buttons( $link, $button, $link2, $button2, $slide_nb ) {
		if (
			empty( $link ) &&
			empty( $button ) &&
			empty( $link2 ) &&
		...
		<div class="buttons">
			...
			if ( ! empty( $link ) || ! empty( $button ) ) {
				...
				echo '<a href="' . esc_url( $link ) . '" title="' . esc_html( $button ) . '" class="btn btn-primary btn-left no-js-color" ' . hestia_is_external_url( $link ) . '>' . wp_kses( $button, $allowed_tags ) . '</a>';
			}
			if ( ! empty( $link2 ) || ! empty( $button2 ) ) {
				$button2 = html_entity_decode( $button2 );
				echo '<a href="' . esc_url( $link2 ) . '" title="' . esc_html( $button2 ) . '" class="btn btn-primary btn-right no-js-color" ' . hestia_is_external_url( $link2 ) . '>' . wp_kses( $button2, $allowed_tags ) . '</a>';
			}
					...
					<div class="container">
						...
						<div class="row">
							...
							<div class="<?php echo esc_attr( $alignment_class ); ?>">
								...
								$this->render_slide_buttons( $link, $button, $link2, $button2, $counter );
