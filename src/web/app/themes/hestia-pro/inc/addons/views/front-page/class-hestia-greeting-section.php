...
/**
 * Class Hestia_Greeting_Section
 */
class Hestia_Greeting_Section extends Hestia_Abstract_Main {
	/**
	 * Initialize.
	 */
	...
	private function hook_section() {
		$section_priority = apply_filters( 'hestia_section_priority', 10, 'hestia_greeting' );
		add_action( 'hestia_sections', array( $this, 'do_section' ), absint( $section_priority ) );
		add_action( 'hestia_do_greeting_section', array( $this, 'render_section' ) );
	}
	...
	public function do_section() {
		do_action( 'hestia_do_greeting_section', false );
	}
	...
	public function render_section( $is_shortcode = false ) {
		 ...
		 */
		$hide_section             = get_theme_mod( 'hestia_greeting_hide', false );
		...
		$default_content          = current_user_can( 'edit_theme_options' ) ? Hestia_Defaults_Models::instance()->get_greeting_default() : false;
		$hestia_greeting_title    = get_theme_mod( 'hestia_greeting_title', $default_title );
		$hestia_greeting_subtitle = get_theme_mod( 'hestia_greeting_subtitle', $default_subtitle );
		$hestia_greeting_content  = get_theme_mod( 'hestia_greeting_content', $default_content );
		$section_is_empty         = empty( $hestia_greeting_content ) && empty( $hestia_greeting_subtitle ) && empty( $hestia_greeting_title );
		$section_style            = '';
		...
		hestia_before_greeting_section_trigger();
		?>
		<section class="hestia-greeting <?php echo esc_attr( $wrapper_class ); ?> aos-animate" id="greeting"
				data-sorder="hestia_greeting" <?php echo $section_style; ?>>
			<?php
			hestia_before_greeting_section_content_trigger();
			if ( $is_shortcode === false ) {
				hestia_display_customizer_shortcut( 'hestia_greeting_hide', true );
			}
			?>
			<div class="<?php echo esc_attr( $container_class ); ?>">
				<?php
				hestia_top_greeting_section_content_trigger();
				if ( $is_shortcode === false ) {
					?>
					<div class="row">
						<div class="col-md-8 col-md-offset-2 hestia-greeting-title-area">
							<?php
							if ( ! empty( $hestia_greeting_title ) && ! empty( $hestia_greeting_subtitle ) ) {
								hestia_display_customizer_shortcut( 'hestia_greeting_title' );
							}
							if ( ! empty( $hestia_greeting_title ) || is_customize_preview() ) {
								echo '<h2 class="hestia-title">' . wp_kses_post( $hestia_greeting_title ) . '</h2>';
							}
							if ( ! empty( $hestia_greeting_subtitle ) || is_customize_preview() ) {
								echo '<h5 class="description">' . wp_kses_post( $hestia_greeting_subtitle ) . '</h5>';
							}
							?>
						</div>
					</div>
				...
				$this->features_content( $hestia_greeting_content );
				?>
				<?php hestia_bottom_greeting_section_content_trigger(); ?>
			</div>
			<?php hestia_after_greeting_section_content_trigger(); ?>
		</section>
		<?php
		hestia_after_greeting_section_trigger();
	}
	 ...
	 * @access public
	 *
	 * @param string $hestia_greeting_content Section content in json format.
	 * @param bool   $is_callback Flag to check if it's callback or not.
	 */
	public function features_content( $hestia_greeting_content, $is_callback = false ) {
		if ( ! $is_callback ) {
			?>
			<div class="hestia-greeting-content">
			<?php
		}
		if ( ! empty( $hestia_greeting_content ) ) :

			$hestia_greeting_content = json_decode( $hestia_greeting_content );
			if ( ! empty( $hestia_greeting_content ) ) {
				echo '<div class="row"' . hestia_add_animationation( 'fade-up' ) . '>';
				foreach ( $hestia_greeting_content as $features_item ) :
					...
					?>
					<div class="col-xs-12 <?php echo apply_filters( 'hestia_greeting_per_rowcol-xs-12_class', 'col-md-4' ); ?> feature-box">
