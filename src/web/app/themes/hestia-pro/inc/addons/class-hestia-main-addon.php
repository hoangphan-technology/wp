...
class Hestia_Main_Addon extends Hestia_Abstract_Main {
	...
	public function init() {
		...
		add_action( 'hestia_about_page_array', array( $this, 'filter_about_page_array_pro' ) );
	...
	public function get_license_type() {
		$status = get_option( 'hestia_pro_license_data' );
		...
		foreach ( $this->tier_map as $tier_id => $price_ids_array ) {
			if ( in_array( $status->price_id, $price_ids_array ) ) {
				return (int) $tier_id;
			}
		}
	...
	public function add_remote_demos( $demos ) {
		$lic_type = $this->get_license_type();
		...
		$addon_demos = array_merge( $remote_demos, $demos['remote'] );
